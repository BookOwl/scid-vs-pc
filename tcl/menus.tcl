...
$m add command -label OptionsSave -command {
  ...
  foreach i {boardSize boardStyle language ::pgn::showColor 
    ...
    ::comp(timecontrol) ::comp(seconds) ::comp(minutes) ::comp(incr) ::comp(timeout) ::comp(name) ::comp(rounds) ::comp(showclock) 
    ...
    puts $optionF ""
    foreach i {lite dark whitecolor blackcolor highcolor bestcolor bgcolor \
    ...
    # save FICS config
    foreach i { use_timeseal timeseal_exec port_fics port_timeseal login password consolebg consolefg silence shouts server_ip} {
